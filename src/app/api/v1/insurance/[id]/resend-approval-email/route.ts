...
export async function POST(
  ...
  try {
    ...
    const target = `${JUICE_API_URL.replace(/\/$/, "")}/api/v1/insurance/${encodeURIComponent(id)}/resend-approval-email`;
    ...
    return NextResponse.json(
      {
        status: "error",
        code: 500,
        message: error instanceof Error ? error.message : "Failed to proxy resend approval email to Juice",
      },
      { status: 500 }
    );
  }
